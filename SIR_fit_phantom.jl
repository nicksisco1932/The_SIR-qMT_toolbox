...
function main()
    ...
    function reshape_and_normalize_phantom(data_4d::Array{Float64},TI::Array{Float64},TD::Array{Float64},NX,NY,NZ,NT)
        # [pmf R1f Sf M0f]
        X=hcat(TI,TD)
        ...
        return tot_voxels,tmpOUT,X,Yy
    end

    tot_voxels,pre_zeros,times,Yy = reshape_and_normalize_phantom( DATA,ti_times,td_times,nx,ny,nz,nt);
    ...
    function work(f::Function,xvalues::Array{Float64},yvalues::Array{Float64},guesses::Array{Float64},n::Int64,fitdata::Array{Float64})
        Threads.@threads for ii in 1:n
            @inbounds fitdata[ii,:] = nlsfit(f,xvalues,yvalues[ii,:],guesses)
            # yvalues[ii,:];
        end    
        return fitdata
    end

    @time tmpOUT = work(SIR_signal_absolute,times,Yy,X0,tot_voxels,tmpOUT);
